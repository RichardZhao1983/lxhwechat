...
import com.lxh.wechat.solmanoa2.trace.OAuthTracer;
...
public class LocalSamlTokenFactory implements SamlTokenFactory {
	...
	private Credential getSigningCredential(Properties _cfg) throws IOException, KeyStoreException,
		...
		if (this._signingCredential == null) { // check configuration
			...
			System.out.println(getCfg(_cfg, CFG_KEYSTORE_PATH));
			InputStream in = getClass().getResourceAsStream(getCfg(_cfg, CFG_KEYSTORE_PATH));
			ks.load(in,getCfg(_cfg, CFG_KEYSTORE_PASSWORD).toCharArray());
			// load key data
			PrivateKey pk = (PrivateKey) ks.getKey(getCfg(_cfg, CFG_KEYSTORE_ALIAS),
					getCfg(_cfg, CFG_KEYSTORE_PASSWORD).toCharArray());

			String keyName = getCfg(_cfg, CFG_KEYSTORE_ALIAS);
			X509Certificate pubKey = (X509Certificate) ks.getCertificate(keyName);
	...
	private Assertion createAssertion(Properties _cfg) throws MissingPropertyException, ConfigurationException {
		...
		authnContextClassRef.setAuthnContextClassRef(getCfg(_cfg, CFG_SAML_AUTHNCONTEXT_PREVIUOUS_AUTHENTICATION,
